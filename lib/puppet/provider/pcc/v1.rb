...
Puppet::Type.type(:pcc).provide(:v1) do
  ...
  def self.instances
    datacenters = Ionoscloud::DataCentersApi.new.datacenters_get(depth: 1).items
    pccs = []
    PuppetX::IonoscloudX::Helper.pccs_api.pccs_get(depth: 1).items.each do |pcc|
  ...
  def description=(value)
    ...
    _, _, headers = PuppetX::IonoscloudX::Helper.pccs_api.pccs_patch_with_http_info(@property_hash[:id], description: value)
  ...
  def create
    ...
    pcc, _, headers = PuppetX::IonoscloudX::Helper.pccs_api.pccs_post_with_http_info(pcc)
    PuppetX::IonoscloudX::Helper.wait_request(headers)
    ...
    if resource[:peers]
      ...
      resource[:peers].each do |peer|
        ...
        _, _, headers = PuppetX::IonoscloudX::Helper.lans_api.datacenters_lans_patch_with_http_info(datacenter_id, peer_id, pcc: pcc.id)
  ...
  def destroy
    if @property_hash[:peers]
      ...
      @property_hash[:peers].each do |peer|
        _, _, headers = PuppetX::IonoscloudX::Helper.lans_api.datacenters_lans_patch_with_http_info(peer['datacenter_id'], peer['id'], pcc: nil)
        headers_list << headers
      end
      headers_list.each { |headers| PuppetX::IonoscloudX::Helper.wait_request(headers) }
    end
    _, _, headers = PuppetX::IonoscloudX::Helper.pccs_api.pccs_delete_with_http_info(@property_hash[:id])
    PuppetX::IonoscloudX::Helper.wait_request(headers)
