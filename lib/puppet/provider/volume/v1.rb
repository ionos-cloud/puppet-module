...
Puppet::Type.type(:volume).provide(:v1) do
  ...
  def self.instances
    PuppetX::IonoscloudX::Helper.datacenters_api.datacenters_get(depth: 1).items.map { |datacenter|
      ...
      unless datacenter.properties.name.nil? || datacenter.properties.name.empty?
        PuppetX::IonoscloudX::Helper.volumes_api.datacenters_volumes_get(datacenter.id, depth: 1).items.each do |volume|
  ...
  def create
    ...
    volume, _, headers = PuppetX::IonoscloudX::Helper.volumes_api.datacenters_volumes_post_with_http_info(datacenter_id, volume)
  ...
  def destroy
    _, _, headers = PuppetX::IonoscloudX::Helper.volumes_api.datacenters_volumes_delete_with_http_info(@property_hash[:datacenter_id], @property_hash[:id])
    PuppetX::IonoscloudX::Helper.wait_request(headers)
