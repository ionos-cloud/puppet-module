...
Puppet::Type.type(:datacenter).provide(:v1) do
  ...
  def self.instances
    PuppetX::IonoscloudX::Helper.ionoscloud_config
    datacenters = []
    Ionoscloud::DataCentersApi.new.datacenters_get(depth: 1).items.each do |dc|
  ...
  def create
    ...
    datacenter, _, headers = Ionoscloud::DataCentersApi.new.datacenters_post_with_http_info(datacenter)
  ...
  def flush
    ...
    _, _, headers = Ionoscloud::DataCentersApi.new.datacenters_patch_with_http_info(@property_hash[:id], changes)
    PuppetX::IonoscloudX::Helper.wait_request(headers)
  ...
  def destroy
    ...
    _, _, headers = Ionoscloud::DataCentersApi.new.datacenters_delete_with_http_info(@property_hash[:id])
    PuppetX::IonoscloudX::Helper.wait_request(headers)
